      ...
      <li v-for="(item,index) in shopcarList" :key="index">
        ...
        <div class="info">
          ...
          <p>
            ...
            <span>
              <van-stepper v-model="item.count" @change="numberChange(item.id,item.count)"/>
            </span>
          </p>
          <p>
            <van-button size="mini" type="danger" @click="delFromShopcar(item.id)">删除</van-button>
          </p>
        </div>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    numberChange(id, number) {
      this.$http
        .post("v1/cart/postGoodsToCart/" + id, { count: number })
        .then(res => {
          if (res.data.status == 200) {
            this.getShopcarList();
          }
